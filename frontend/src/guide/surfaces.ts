...
export const SURFACES: SurfaceDescriptor[] = [
  ...
  {
    path: '/w/:workspace/agents/:slug/work',
    title: 'Agent work',
    audience: 'Anyone in the workspace',
    what: "What this agent is working on, and where an agent opens. The board grouped by who has the ball, or by project (each project's Drive folder, outcome and open counts as the group header) — grouping is a toggle, not a separate page. A task that is asking you something says so on its card and can be decided there. Settled work is hidden until asked for, and `?batch=` opens one sitting, e.g. a fleet audit.",
    actions: ['Group by status or by project', 'Show settled work', 'Accept, decline, dispatch or complete a task', 'Decide an ask on the card', 'Add a project', 'Dispatch a one-off prompt'],
  },
  {
