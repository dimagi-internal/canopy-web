...
import { runnerAlerts, type RunnerAlert } from './runnerAlertRules'

// LOUD alert: something is wrong with this runner. Two faults, ranked in
// runnerAlertRules.ts — its source checkout is on the wrong branch, or its
// install is behind what shipped. (A runner silent for a day raises nothing:
// most of the fleet is expected to be dark.) A HEARTBEATING runner is fixed
...
function headline({ kind, unreachable }: RunnerAlert): string {
  if (kind === 'branch') {
    return unreachable
