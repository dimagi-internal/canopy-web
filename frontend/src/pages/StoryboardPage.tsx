...
function Board({ board, token }: { board: Storyboard; token: string | null }) {
  ...
  // One overlay for the page, not one per card: two open players would both be
  // playing audio.
  const [expanded, setExpanded] = useState<
    Storyboard['acts'][number]['entries'][number] | null
  >(null)
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="mx-auto max-w-3xl px-6 py-12 md:py-16">
        ...
        {expanded && <Lightbox entry={expanded} onClose={() => setExpanded(null)} />}
...
/** The expanded player: big, but still a page.
 *
 *  Element fullscreen was the first answer and the wrong one — it blanks the
 *  whole display, hides the browser, and drops the viewer somewhere that looks
 *  like a different application. A demo is watched next to the email that sent
 *  it. This grows the video to the window and leaves the page around it, so
 *  Escape or a click outside puts you back where you were.
 */
function Lightbox({
  entry,
  onClose,
  ...
  useEffect(() => {
    ...
    // The page behind must not scroll under the overlay — a trackpad flick
    // while the pointer is off the video otherwise moves the thing you came
    // back to.
  ...
  return (
    ...
    >
      <button
        ...
        className="absolute right-4 top-4 rounded-md px-3 py-1.5 text-sm text-white/80 hover:bg-white/10 hover:text-white"
      >
        Close ✕
      </button>
      <video
        ...
        className="max-h-[85vh] w-full max-w-6xl rounded-lg bg-black shadow-2xl"
        src={withBase(entry.video_url ?? '')}
        // The backdrop closes; the video itself must not, or every play/pause
        // click would shut the thing the viewer just opened.
        onClick={(e) => e.stopPropagation()}
      />
      <p className="mt-3 max-w-6xl text-center text-sm text-white/70">{entry.title}</p>
    </div>
  )
}
    ...
    >
      ...
      >
        {entry.published && entry.video_url ? (
          reel ? (
            // No inline controls on a reel: a click on a `controls` video
            // toggles play, so opening the overlay would ALSO start the
            // thumbnail playing behind it. The card is a poster with one job.
            <button
              type="button"
              onClick={onExpand}
            ...
            >
              ...
              <span className="absolute inset-0 flex items-center justify-center bg-black/20 transition-colors group-hover:bg-black/35">
                <span className="flex h-14 w-14 items-center justify-center rounded-full bg-black/60 text-2xl leading-none text-white shadow-lg">
                  ▶
                </span>
              </span>
            </button>
