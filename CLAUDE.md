...
- `POST /api/inbound/gmail/` — Gmail Pub/Sub push. `auth=None` and self-enforcing on a Google-signed **OIDC JWT** (`google-auth` was already a dependency), following `POST /api/auth/token-exchange`'s precedent and allowlisted in `apps/common/middleware.py`.

**It is a doorbell, not a reader.** A Gmail push carries `{emailAddress, historyId}` and **no content**, so something must still call Gmail — and only the runner holds the per-agent `gog` OAuth clients. So the receiver resolves mailbox → agent → assigned runners and sends a `runner.check_inbox` control frame (a sixth frame type on the channel already carrying `wake`/`interject`/`cancel`/`stream`/`menu_answer`); the runner does the read it already does. No mail credential enters the web app, the existing idempotency key and "agent's own reply" skip are untouched, and **a forged doorbell cannot inject a fake email** — the worst it does is cause a `gog` read that finds nothing.

Two response rules that look wrong and aren't: an unverified push gets **404, not 403** (a probe learns nothing), and a push we can't act on still gets **200** — a 4xx tells Pub/Sub to REDELIVER, so a mailbox we don't own would be retried forever. The refusal belongs in the event log, not on the wire. Runner selection composes `harness.assignment_rows_for(..., ORIGIN_EMAIL, ...)` — the same helper `claim_next_turn` uses — so a strict `email` source rule is honoured and the doorbell can never ring a box that routing would refuse the turn to. Every online assigned runner is rung, not just rank 0: the enqueue is idempotent per `(thread, messageCount)`, so a duplicate read collapses server-side, and ringing only the top rank would land nothing in exactly the case the cascade is about to promote a lower one.
...
- `POST /api/inbound/watch/` — caller-authed (session or PAT; this is our fleet reporting state, not Google calling in). Whatever re-armed a mailbox's Gmail watch reports the new expiry here, which is what makes the 7-day cliff **loud**: with an expiry on file the log says `gmail.watch.expiring` a day out and `gmail.watch.expired` after. A null expiry means "no watch" — distinct from never having reported, and it silences the push-miss auditor, since with no watch there is nothing to have missed. 404s (never 403s) for a non-member: whether an address is registered isn't something a stranger gets to learn.

`INBOUND_PUSH_AUDIENCE` empty means the endpoint **refuses everything** (an unconfigured deployment must not quietly accept anonymous pushes; refusing costs latency, not mail). Set `INBOUND_PUSH_SERVICE_ACCOUNT` too — audience alone is not identity. **A Gmail `watch` expires within 7 days and Google won't renew it**, so the **runner** re-arms it (`gmail_watch.py`, 24h before expiry, on the tick it already runs) and reports the expiry to `/api/inbound/watch/`. It must be the runner, not the server: `users.watch` is called AS the mailbox, which a service account can only do via **domain-wide delegation** — and `dimagi-ai.com` is a *secondary domain inside the dimagi.com Workspace org* (`C018tavmm`, established in PR #151), not its own tenant, so DWD there would cover every `dimagi.com` mailbox. The runner's five per-mailbox OAuth grants are strictly narrower and need no admin-console change; since `gog` has no `watch` verb and won't print a bearer, it does the refresh-token exchange itself from gog's own client file + keyring export, storing nothing new. Set `gmail_watch_topic` in `runner.json` to enable — empty means the runner never touches `users.watch`. Provisioning is `docs/runbooks/gmail-push-provisioning.md`. With no watch registered, push is inert and mail is found by the poll — today's behaviour — which is why shipping this without the GCP side is safe.
