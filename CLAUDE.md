...
- **The PRIMARY producer is the transcript, not the screen** (`canopy_transcript.pending_question`, spec 2026-07-31). What actually blocks this fleet is **AskUserQuestion** (see the bullet below), and that is a *tool call* — so the question, the options and their descriptions are already in the file the runner tails. No CDP, so no stolen focus; 0.31 ms per session, so it runs for EVERY open session on the ordinary ~10s report rather than only where a viewer is attached; and it is recomputable, so a menu survives being looked at later. The screen read stays for what a transcript cannot see (permission prompts, trust gates). The three producers emit the identical dict; `source` says which. Numbers are derived for the DECLARED options only — Claude Code renders those first, from 1, then appends its own — and `answer_menu_with` still re-reads the real screen and refuses an option not on it, so a divergence costs a dropped tap, never a wrong keypress. **`null` is a real answer and is written through**: the report is a fresh observation, so "no dialog" is what retires a menu somebody answered at the laptop, and buttons that outlive their dialog type a number into an ordinary prompt.
