...
- **Infra vs release: one writer each (and today, two resources with two writers).** CloudFormation owns the DURABLE resources — ECR repo, log group, secrets, target group, listener rule, the attachments S3 bucket — applied rarely and deliberately. The deploy workflow owns the RELEASE: it registers a task-def revision and calls `update-service`. The rule to keep in mind: **releases never run CloudFormation, and CloudFormation never touches a running service.** Putting CFN in the deploy path would serialize every deploy behind a stack update, which is wrong for a repo where parallel agents ship constantly.
  **The known defect:** CFN still *declares* `TaskDefinition` + `Service`, so those two resources have two writers. CFN's `ImageTag` parameter freezes at the last apply while the workflow moves the service on — observed 2026-07-26 at a **ten-day, 118-revision** gap. A plain `aws cloudformation deploy` re-registers the task def from the stale parameter and points the service at it: a silent rollback, reported as success. **Never run `aws cloudformation deploy` against this stack.** Use `deploy/aws/apply-stack.sh`, which reads the running image, pins `ImageTag` to it, shows the resource diff and asks before executing. (It also passes `UsePreviousValue` for every other parameter — `deploy` substitutes template *defaults*, and `DesiredCount` defaults to **0**, which would scale the site to zero.) The real fix is to remove `TaskDefinition` + `Service` from the template (retain-then-remove) so each resource has one writer; the script is the safety net until then.
