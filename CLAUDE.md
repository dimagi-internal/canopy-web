...
`INBOUND_PUSH_AUDIENCE` empty means the endpoint **refuses everything** (an unconfigured deployment must not quietly accept anonymous pushes; refusing costs latency, not mail). Set `INBOUND_PUSH_SERVICE_ACCOUNT` too — audience alone is not identity. **A Gmail `watch` expires within 7 days and Google won't renew it**; provisioning + the weekly re-arm are `docs/runbooks/gmail-push-provisioning.md` until the runner automates it. With no watch registered, push is inert and mail is found by the poll — today's behaviour — which is why shipping this without the GCP side is safe.
