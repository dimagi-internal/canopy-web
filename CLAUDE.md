...
- **The PRIMARY producer is the HOOK. The transcript cannot see a live dialog at all** (`canopy_transcript.menu_from_hook`; corrects spec 2026-07-31, which named `pending_question` primary). What actually blocks this fleet is **AskUserQuestion** (see the bullet below), and that is a *tool call* — so the whole menu is structured data rather than pixels, which is the part that spec got right. What it got wrong is *when the file gets it*: **Claude Code writes the `tool_use` record only once the ask is ANSWERED.** Measured 2026-08-01 across 60 transcripts on a live box — 39 `AskUserQuestion` records, every one already answered, **zero pending** — while two sessions sat blocked on visible dialogs with nothing in their files. So `pending_question` reports dialogs that are over and never one that is waiting, which is the exact inverse of the job; it stayed *green* the whole time, because "no pending question" is indistinguishable from "nothing is blocked". `PreToolUse` fires at the other end of the same tool call and carries the identical `tool_input` — same parse, same dict, still no CDP and no stolen focus, still computed for every open session on the ordinary ~10s report. The transcript reader is kept and consulted SECOND (it wins on a session whose hooks were never installed); the screen read stays for what neither can see (permission prompts, trust gates). All three emit the identical dict; `source` says which. Numbers are derived for the DECLARED options only — Claude Code renders those first, from 1, then appends its own — and `answer_menu_with` still re-reads the real screen and refuses an option not on it, so a divergence costs a dropped tap, never a wrong keypress. **`null` is a real answer and is written through**: the report is a fresh observation, so "no dialog" is what retires a menu somebody answered at the laptop, and buttons that outlive their dialog type a number into an ordinary prompt.
- **The screen parser's footer requirement is not sufficient on its own — a tall dialog has no room to draw one.** `menu.find_menu` fails closed on consecutively-numbered options plus a footer offering a way out, because numbered lines are the most common shape in an agent's own output. But the footer is only drawn if a row is left to draw it on: a long question with six options each carrying a description overflows a short emdash pane, and the line that falls off the bottom is the footer. Captured live 2026-08-01 from two sessions stuck exactly this way (15 min and 1h20m), 41-row frames ending mid-dialog on `6. Chat about this`. So there is a second, equally strict acceptance: the TUI's **selection cursor** (`❯`) sits on one of the options **and** the dialog runs to the bottom of the frame. Both halves are load-bearing — prose does not draw `❯` at the head of a numbered line, and a live dialog is always the last thing on screen (the TUI draws it where the composer would be), so a menu an agent merely *quoted* has its own output below it and is still rejected. The same frames also showed the question arriving as its own tail (`"press into that session?"`), so a question hard-wrapped by the grid is rejoined — only on the wrap signature (ran to the frame width, broke at a space), because a dialog's SUBJECT sits directly above its question and swallowing one into the other misreports what is being asked.
- **A tap's outcome is reported, never just logged — and the menu stays up until it lands.** `POST .../answer-menu` returns `sent` the instant it relays the control frame; it cannot know whether the keystroke arrived, because the runner presses it. So every refusal on the runner side is now a value (`answered` / `no_dialog` / `not_on_menu` / `wrong_pane` / `unreachable`) that rides the next session report back inside the menu itself (`answer_error` + a human sentence in `answer_note`; no migration — `RunnerBinding.pending_question` is JSON). A success drops the menu at once rather than waiting for the agent's `PostToolUse`; a refusal KEEPS it, so the phone shows the same buttons plus the reason they did not work. The report is forced (`sessions.request_report_now`) because a blocked session writes nothing, so the change-driven tick would never fire and the answer would wait out the whole heartbeat. This is the last mile of "clicking does nothing": with every cause underneath it fixed, a correct refusal was still indistinguishable from a press that worked (labs, 2026-08-01 — 45 minutes of taps dying on `NOT_A_CLAUDE_PANE` in silence). **`wrong_pane` stays a refusal on purpose**: clicking the Claude tab for somebody means guessing at emdash's tab controls, and the cost of guessing wrong is a digit executed in their shell — so it says which tab to switch to instead.
- **A pending menu is persisted, because the hook that raised it fires once.** `~/.canopy/pending-menus.json` (`menu_store.py`), written on every change and reloaded at startup. Without it a restart does not merely FORGET a live dialog — the next report ships `question: null`, which RETIRES it server-side, and nothing can rediscover it: no hook re-fires and the transcript will not carry the ask until it is answered. The runner auto-updates every 30 minutes, so a restart is routine. A restored menu may be stale, and that is handled where it belongs: a tap re-reads the real screen, gets `no_dialog`, and both says so and clears it — rather than a CDP verification at startup, which would CLICK the task and steal focus (the thing #510 was reverted for).
