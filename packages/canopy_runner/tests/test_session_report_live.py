...
from canopy_runner import emdash, transcript
...
def test_reports_on_growth_skips_idle_and_heartbeats(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(emdash, "list_open_sessions", lambda _db, limit=30: sessions)
    ...
    sessions_mod.maybe_report_sessions(_Cfg(), c, now_fn=now)
    assert c.reports == 1
...
def test_a_failed_emdash_read_skips_the_report_entirely(tmp_path, monkeypatch):
    ...
    def _boom(_db, *_a, **_k):
        raise emdash.EmdashReadError("schema drift")

    monkeypatch.setattr(emdash, "list_open_sessions", _boom)
    c = _Client()
...
def test_report_honours_the_configured_limit(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(emdash, "list_open_sessions", _list)
    monkeypatch.setattr(transcript, "attach_recent_tail", lambda _s, **_k: None)
    sessions_mod.maybe_report_sessions(_Cfg(), _Client(), now_fn=lambda: 100.0)
...
def test_report_carries_archived_task_names(tmp_path, monkeypatch):
    sessions_mod._tail_readers.clear()
    sessions_mod._last_session_report = 0.0
    monkeypatch.setattr(emdash, "list_open_sessions", lambda _db, _l=100: [])
    monkeypatch.setattr(emdash, "list_recently_archived_tasks", lambda _db, _l=100: ["gone"])
    monkeypatch.setattr(transcript, "attach_recent_tail", lambda _s, **_k: None)
    c = _Client()
    sessions_mod.maybe_report_sessions(_Cfg(), c, now_fn=lambda: 100.0)
...
def test_a_failed_archived_read_still_reports_open_sessions(tmp_path, monkeypatch):
    ...
    def _boom(_db, _l=100):
        raise emdash.EmdashReadError("drift")

    monkeypatch.setattr(emdash, "list_open_sessions", lambda _db, _l=100: [])
    monkeypatch.setattr(emdash, "list_recently_archived_tasks", _boom)
    monkeypatch.setattr(transcript, "attach_recent_tail", lambda _s, **_k: None)
    c = _Client()
    sessions_mod.maybe_report_sessions(_Cfg(), c, now_fn=lambda: 100.0)
    assert c.reports == 1
