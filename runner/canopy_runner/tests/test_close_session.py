...
def test_a_close_queued_mid_post_survives_the_report(monkeypatch):
    """`_PENDING_CLOSED` is written from the wake-listener thread and read/cleared
    from the report-tick thread. A name added while the POST is in flight (a close
    that lands mid-round-trip) must not be wiped by a wholesale `.clear()` — it has
    to ride the NEXT report instead, since emdash has already deleted the task and
    ...
    class _Client:
        def report_sessions(self, runner_id, payload, archived=None):
            ...
            sessions._PENDING_CLOSED.add("late")
    ...
    sessions.maybe_report_sessions(cfg, _Client(), now_fn=lambda: 0.0)
    assert sent["archived"] == ["ddd"]
    # "late" arrived after this report's payload was built — it must still be
    # queued, ready to ride the next one.
    assert sessions._PENDING_CLOSED == {"late"}
