...
| `feedback` | **framework** | Feedback on a thing, from a person, via a channel (`web`/`email`/`gdoc`/`manual`/`api`). Generic over its target (`target_kind` + `target_ref` strings, never an FK) so it imports no product app — the same discipline `Item` follows. Idempotent per `(channel, source_ref)` so re-reading a mailbox is safe. Deliberately emits NO signal: feedback is input to a decision, not work — a turn reads the pool when the owner is ready. canopy-web runs no pollers and holds no third-party credentials; agents ingest. |
| `events` | **framework** | A durable, fleet-wide log of actions and errors — the place an operational fault lives when it belongs to no `Turn`. Generic over its producer (`source` + `kind` strings, never FKs), so it imports nothing and any app may write to it. Repeats of `(workspace, source, key)` coalesce onto one row with a `count` — the runner's in-memory `failure_log` streak idea made durable and server-visible. `workspace` is NOT NULL (the nullable-tenant-FK fail-open class). Deliberately emits NO signal, like `feedback`: an event is a record, not work — promoting faults into `Item`s would both misuse `Item`'s decision set and turn a flapping runner into a Web Push storm. Read-only after write; there is no mutation route, because a log you can edit is not a record. |
| `slack` | **framework** | The Slack front door: a Slack thread becomes a chat session with an agent whose owner turned Slack on, sent as `origin="slack"` by the linked canopy user. Holds the bot token (encrypted); agents never do. Grants nothing — an unlinked or non-member Slack user gets words back and no turn. See `docs/superpowers/specs/2026-09-18-slack-front-door-design.md`. |
| `inbound` | **framework** | The push doorbell: receives a Gmail Pub/Sub notification and rings the runner that holds the mailbox credentials. **Never reads mail** — a Gmail push carries `{emailAddress, historyId}` and no content, and only the runner has the per-agent `gog` OAuth clients, so no mail credential enters the web app and a forged ping can at worst cause a read that finds nothing. **Per-workspace by construction:** the tenant is named in the URL (`/api/inbound/gmail/{workspace}/`) and verification binds to that workspace's own `InboundPushConfig` (audience + signer + watch topic), so a second tenant in its own GCP project verifies correctly and one tenant's service account can never satisfy another's check — a *list* of allowed signers would not do that. Naming the tenant in the path also means it is known before the body is decoded, so no attacker-controlled byte selects the credential. `InboundMailbox` maps address → agent (explicit data, not an `@`-split convention whose failure is silent); tenancy derives one hop away via the agent, the same shape `Turn` uses. Composes its runner list with `harness.assignment_rows_for`, the helper claiming uses, so the doorbell can't ring a box routing would never give the turn to. Unlike `events` this app DOES have a signal — a `post_save` audit that writes a log row when the 300s poll finds mail push should have rung for — but it only ever writes a row, never work. All configuration is first-class records edited at `/w/:workspace/inbound`; there are deliberately no deployment-global settings. |
