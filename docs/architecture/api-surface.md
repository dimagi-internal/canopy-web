...
- `GET|POST /api/agents/{slug}/turns/` — List / package a turn: the request(s) it advanced (`task_ext_ids`) → what it did (`title`/`summary`) → deliverables (`work_product_urls`) → optionally a `/share/<token>` transcript link (uploaded to the sessions app; the turn only holds its `slug`/`share_token`, so the apps stay decoupled). Idempotent per `(agent, cli_session_id)`. Drives the **Turns** rail section.
  POST attaches the report to the `harness.Turn` that dispatched the session, matched on `emdash_task_id` (the runner stamps it at finish; the closing agent recovers it from its cwd). Unmatched — a session a human started by hand — still records, as a report-only turn. There is no separate `AgentTurn` model: dispatch and close-out are two halves of one row (merged 2026-08-11; they were two tables serving the same word, and the detail endpoint read the half that was usually empty)
