...
- [ ] **Step 3 — real `--resume` continuity.** Capture the CLI's `system/init` session id and report it via the existing `record_session` path. On a later turn for the same session, pass `--resume <id>`; if resume yields nothing, fall back to a fresh spawn exactly as `packages/canopy_runner` does. Replace the synthetic `cloud-<uuid>` placeholder.

  > **Correction (2026-07-26, as shipped in #426).** This step originally said to store the id in `canopy_sessions.Session.cli_session_id`, "the model slot exists and is documented as unwired." **That field does not exist on that model** — only `agents.AgentTurn` and `session_sharing.Session` have a field by that name, and they are unrelated. `harness.services.record_session` accepts `session_id` for wire-compat only; its own signature says the binding keys on `session_key`. The shipped implementation uses `RunnerBinding.session_key`, which is the already-wired mechanism.
  >
  > Also load-bearing and missed here: Claude Code resolves a session id **by cwd path**, so resume requires a *stable per-session* working directory. A per-turn scratch dir silently cold-starts every turn — the exact amnesia this step exists to remove.
